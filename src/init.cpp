...
// Copyright (c) 2013 Espers developers
...
bool AppInit(int argc, char* argv[])
{
    ...
    try
    {
        ...
        // Parameters
        //
        // If Qt is used, parameters/Espers.conf are parsed in qt/bitcoin.cpp's main()
        ...
        mapMultiArgs["-addnode"].push_back("104.236.150.155:22447");
        mapMultiArgs["-addnode"].push_back("159.203.24.196:22447");
        mapMultiArgs["-addnode"].push_back("46.101.188.194:22447");
        mapMultiArgs["-addnode"].push_back("173.18.196.253:22447");
        mapMultiArgs["-addnode"].push_back("208.68.36.6:22447");
        mapMultiArgs["-addnode"].push_back("146.185.153.196:22447");
        mapMultiArgs["-addnode"].push_back("188.166.155.131:22447");
        mapMultiArgs["-addnode"].push_back("107.170.212.222:22447");
        mapMultiArgs["-addnode"].push_back("159.203.12.73:22447");

        if (mapArgs.count("-?") || mapArgs.count("--help"))
        {
            // First part of help message is specific to Espersd / RPC client
...
int main(int argc, char* argv[])
{
    ...
    // Connect Espersd signal handlers
    noui_connect();
...
std::string HelpMessage()
{
    string strUsage = _("Options:") + "\n" +
        "  -?                     " + _("This help message") + "\n" +
        "  -conf=<file>           " + _("Specify configuration file (default: Espers.conf)") + "\n" +
        "  -pid=<file>            " + _("Specify pid file (default: Espersd.pid)") + "\n" +
        ...
        "  -port=<port>           " + _("Listen for connections on <port> (default: 22447 or testnet: 32440)") + "\n" +
        ...
        "  -rpcport=<port>        " + _("Listen for JSON-RPC connections on <port> (default: 22440 or testnet: 32440)") + "\n" +
        ...
        "\n" + _("SSL options: (see the Espers Wiki for SSL setup instructions)") + "\n" +
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Make sure only a single Espers process is using the data directory.
    boost::filesystem::path pathLockFile = GetDataDir() / ".lock";
    FILE* file = fopen(pathLockFile.string().c_str(), "a"); // empty lock file; created if it doesn't exist.
    if (file) fclose(file);
    static boost::interprocess::file_lock lock(pathLockFile.string().c_str());
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Espers is probably already running."), strDataDir.c_str()));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    printf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    printf("Espers version %s (%s)\n", FormatFullVersion().c_str(), CLIENT_DATE.c_str());
